...
The FastAPI application is defined in `f1_api/www.py` and includes:

- **Automatic API documentation** at `/docs` (Swagger UI) and `/redoc` (ReDoc)
- **Static file serving** from `f1_api/static/` via the `/static/` endpoint
...
Create a new route in `f1_api/www.py`:
...
Static files are served from `f1_api/static/`:
...
uvicorn f1_api.www:app --reload --host 0.0.0.0 --port 8000
...
uvicorn f1_api.www:app --host 0.0.0.0 --port 8000 --workers 4

# Or using gunicorn with uvicorn workers
gunicorn f1_api.www:app -w 4 -k uvicorn.workers.UvicornWorker
```
